...
if_amount_same(); #spr czy każdy ma tyle samo obstawione, wtedy przechodzi do kolejnej rundy - liczba kart na stole ++ v koniec gry i sprawdzenie kto wygrał

main()
    rand_deck() #wylosowanie dla każdego gracza 2 kart i 3 kart na stół
    while (stol[5][0] != 0)
       while (if_amount_same()=false)
        bot_choice() #powtorzenie dla wszystkich botów, na razie dla testów trzeba zrobić randomowe decyzje
        player_choice() #wybór: fold, call, raise (i ile)
    check_points() #kto wygrał
    player_amount() #zaktualizowanie pieniędzy gracza - jeśli wygrał zgarnia wszystko, pewnie się doda do funkcji powyżej aby było przejrzyściej
